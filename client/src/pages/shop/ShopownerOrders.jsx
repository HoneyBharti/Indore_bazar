...
import { getshoporders } from '../../features/shop/shopslice';
...
const {shop , shoploading , shoperror , shoperrormsg , shoporders , shopsuccess} = useSelector(state=> state.shop)
...
const handleorderdetail=(orderdetails)=>{
  setorderdetails(orderdetails)
  setshoworderdetail(showorderdetail ? false : true)
}
...
{
  showorderdetail && <Orderdetailsmodal handleorderdetail={handleorderdetail} orderdetails={orderdetails} />
}
            ...
            shoporders.map(order=>{
              return(
                ...
                <td className="px-6 py-4">
                  <span className="font-medium text-slate-900">#{order._id}</span>
                </td>
                <td className="px-6 py-4">
                  <div>
                ...
                <td className="px-6 py-4">
                  <span className="text-sm text-slate-700">{order?.products[0]?.qty}</span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <span className={`inline-block px-3 py-1 text-xs font-medium ${order.status == "cancelled" ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700" } rounded-full`}>
                    {order.status == "cancelled" ? "Not Paid" : "Paid"}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <span className={`inline-block px-3 py-1 text-xs font-medium ${order.status == "cancelled" ? "bg-red-100 text-red-700" : "bg-emerald-100 emerald-amber-700"} rounded-full`}>
                    {order.status}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <button onClick={()=> handleorderdetail(order)} className="p-2 hover:bg-slate-100 rounded-lg">
