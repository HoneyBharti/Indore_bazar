...
import { getshopcoupons, getshopdetails, getshoporders, getshopproducts } from '../../features/shop/shopslice';

function ShopDashboard() {

const dispatch = useDispatch()
  const {user} = useSelector(state=> state.auth)
const {shop , shoploading , shoperror , shoperrormsg , shopproducts , shoporders , shopcoupons , shopsuccess} = useSelector(state=> state.shop)


const totalrevenue = shoporders.reduce((acc , order)=> acc + (order.totalbillamount || 0) , 0)
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
          ...
          <div className="space-y-4">
          {
            shoporders.map(order=>{
              ...
              <div>
                <div className="font-medium text-slate-900">#{order._id}</div>
              ...
              <div className="text-right">
                ...
                <div className={`inline-block px-2 py-1 text-xs font-medium rounded-full mt-1 ${order.status == "cancelled" ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"}`}>
                  {order.status}
                </div>
              </div>
            </div>
