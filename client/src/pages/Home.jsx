import { ShoppingBag, MapPin, Clock, Store, Leaf, Zap, TrendingUp, Menu, Apple, Coffee, Candy, Leaf as LeafIcon, UtensilsCrossed, Cake, LucideShoppingBasket, LucideShoppingBag } from 'lucide-react';
...
import { getallproducts, getallshops } from '../features/product/productslice';
...
const {products , shops , productloading , producterror , producterrormsg , productsuccess} = useSelector(state=> state.product)
...
setTimeout(()=>{
    ...
    if(producterror && producterrormsg){
        toast.error(producterrormsg)
    }
}, [producterror , producterrormsg])
                        ...
                        products.map((product , index) =>{
                           if(index >=5){
                            return
                             ...
                             return(
                                   <div key={product._id} className="bg-white rounded-2xl overflow-hidden hover:shadow-xl transition-shadow cursor-pointer">
                            <div style={{backgroundImage : `url(${product.productImage})`}} className="aspect-square bg-cover bg-center bg-gradient-to-br from-orange-100 to-red-100"></div>
                            <div className="p-4">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <Link to={`/products/${product._id}`} className="px-3 py-1.5 bg-emerald-500 text-white text-sm font-medium rounded-lg hover:bg-emerald-600">View</Link>
                                </div>
                            </div>
                        </div>
                            )
                    ...
                    <div className="flex overflow-x-auto gap-6 pb-4 scrollbar-hide lg:grid lg:grid-cols-4">
                       {
                        shops.map((shop , index)=>{
                          if(index <=4){
                            ...
                            <div className="w-16 h-16 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-xl mb-4 flex items-center justify-center text-white font-bold"><LucideShoppingBag/></div>
